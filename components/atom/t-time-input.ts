...
export class TimeInput extends LitElement {
  ...
  private _handleMove(event: MouseEvent | TouchEvent) {
    ...
    // Preserve decimal precision for time input
    const newValue = Math.max(0, this.initialValue + direction * totalValueChange);
    this._value = newValue;
  ...
  render() {
    return html`
      ...
      <div class="value-display">${Number.isInteger(this._value) ? this._value : this._value.toFixed(1)}${this.unit}</div>
