...
test.describe('Dialog & Consent flow', () => {
  test('should load app, OK welcome, consent, screenshot', async ({ page }) => {
    ...
    const consentNotify = page.locator('.notifyjs-wrapper:has(h2:text("Cookie consent"))');
    ...
    await page.screenshot({ path: 'test-artifacts/DCF_Startup_state.png', fullPage: true });
    // Assert: notification is visible in the top right with the correct text
    const notify = page.locator('.notifyjs-corner');
    await expect(notify).toBeVisible();
    await expect(notify).toContainText('Troff is now cached and will work offline');
  });
});
