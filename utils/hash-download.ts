...
import type { TroffData, TroffHistoryList, TroffDataIdObject } from '../types/troff.d.js';
 ...
 */
export async function downloadSongFromHash(hash: string): Promise<string | null> {
  ...
  try {
    await Promise.all([
      fetchAndCacheFile(troffData.fileUrl, troffData.fileName),
